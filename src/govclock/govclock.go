...
// a VClock object.
func FromBytes(value []byte) (vc *VClock, err error) {
	vc = New()
	if len(value) != 0 {
		header := value[0]
		if (header &^ 0x01) != 0 {
			err = errors.New("Bad vclock header")
			return
		}
		...
		for pos != len(value) {
			...
			pos += size
			if !ok || pos >= len(value) {
				err = errors.New("Bad vclock data (ticks)")
				return
			}
			if vc.hasUpdateTime {
				...
				pos += size
				if !ok {
					err = errors.New("Bad vclock data (time)")
					return
				}
			}
			...
			if !ok || (pos+int(idLen)) > len(value) {
				err = errors.New("Bad vclock data (id)")
				return
			}
